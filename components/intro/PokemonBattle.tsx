...
export function PokemonBattle({ onComplete }: { onComplete: () => void }) {
    ...
    useEffect(() => {
        ...
        const runSequence = async () => {
            ...
            if (battleState === 'ENCOUNTER') {
                ...
                // Faint animation
                setHanyangOpacity(0);
                if (scriptActive) await typeText('야생의 하냥이는 쓰러졌다!');
                await new Promise((r) => setTimeout(r, 1000));
    ...
    return (
        <div
            ...
            onClick={handleNext}
        >
            <div className="w-full max-w-4xl aspect-[4/3] bg-[#f8f8d0] relative overflow-hidden border-4 border-gray-700 shadow-2xl">
                ...
                {(battleState === 'EVOLUTION' || battleState === 'EVOLVED') && (
                    <div className="absolute inset-0 z-[60] bg-black">
                        ...
                        <motion.div
                            ...
                            animate={{
                                filter: evolutionPhase === 'flash'
                                    ? ['brightness(0)', 'brightness(10)', 'brightness(0)'] // Black <-> White
                                    : 'brightness(1)',
                            }}
                            // Flash Transition (Fast pulses)
                        ...
                        {/* Full Screen Flash Overlay (Optional, for white-out effect) */}
                        {evolutionPhase === 'flash' && (
                            <motion.div
                                ...
                                {[...Array(100)].map((_, i) => (
                                    <motion.div
                                        ...
                                        style={{
                                            left: '50%',
                                            top: '50%',
                                            clipPath: 'polygon(50% 0%, 100% 50%, 50% 100%, 0% 50%)', // Diamond Star Shape
                                            background: i % 3 === 0 ? '#fbbf24' : '#ffffff', // Gold/White
                ...
                <motion.div
                    initial={{ x: -300 }}
                    animate={{ x: 0 }}
                    className="absolute top-[10%] left-[5%] z-20 w-64 sm:w-80 font-galmuri"
                >
                    {/* Shadow/Border Container */}
                    <div className="relative bg-black/40 p-1" style={{ transform: 'skewX(-15deg)' }}>
                        {/* Main Box */}
                        <div className="bg-[#f8f8f0] border-2 border-[#404040] p-2 shadow-inner relative overflow-hidden">
                            ...
                            <div className="flex justify-between items-baseline mb-1 px-2" style={{ transform: 'skewX(15deg)' }}>
                                <span className="text-black font-extrabold text-lg sm:text-xl tracking-tighter">하냥이</span>
                                <span className="text-black font-bold text-sm">Lv.5</span>
                            </div>
                ...
                <AnimatePresence>
                    {trainerVisible && (
                        ...
                        >
                            <div
                                className="w-[195px] h-[195px] sm:w-[260px] sm:h-[260px]"
                                style={{
                ...
                {battleState !== 'ENCOUNTER' && battleState !== 'TRAINER_APPEAR' && battleState !== 'TRAINER_THROW' && (
                    <motion.div
                        className="absolute bottom-[30%] left-[20%] w-32 h-32 sm:w-48 sm:h-48 z-20"
                ...
                {(battleState !== 'ENCOUNTER' && battleState !== 'TRAINER_APPEAR' && battleState !== 'TRAINER_THROW') && (
                    <motion.div
                        initial={{ x: 300 }}
                        animate={{ x: 0 }}
                        className="absolute bottom-[35%] right-[5%] z-20 w-72 sm:w-96 font-galmuri"
                    >
                        {/* Shadow/Border Container */}
                        <div className="relative bg-black/40 p-1" style={{ transform: 'skewX(-15deg)' }}>
                            {/* Main Box */}
                            <div className="bg-[#f8f8f0] border-2 border-[#404040] p-3 shadow-inner relative overflow-hidden">
                                {/* Dark strip */}
                                <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-[#e8e8e8] to-[#d0d0d0] -z-10"></div>

                                <div className="flex justify-between items-baseline mb-2 px-2" style={{ transform: 'skewX(15deg)' }}>
                                    <span className="text-black font-extrabold text-xl sm:text-2xl tracking-tighter">섭승이</span>
                                    <span className="text-black font-bold text-lg">Lv.{cyndaquilLevel}</span>
                                </div>
                ...
                {!(battleState === 'EVOLUTION' && evolutionPhase === 'intro') && (
                    <div className="absolute bottom-4 left-4 right-4 h-32 bg-[#f8f8f0] border-4 border-[#404040] z-[80] shadow-lg pixel-corners text-black font-galmuri">
                        {battleState === 'MENU' ? (
